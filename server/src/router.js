const Router = (routes = {}) => ({
  get: (urlOrHandler, handler) =>
    Router(routes).register("GET", urlOrHandler, handler),
  post: (urlOrHandler, handler) =>
    Router(routes).register("POST", urlOrHandler, handler),
  put: (urlOrHandler, handler) =>
    Router(routes).register("PUT", urlOrHandler, handler),
  patch: (urlOrHandler, handler) =>
    Router(routes).register("PATCH", urlOrHandler, handler),
  delete: (urlOrHandler, handler) =>
    Router(routes).register("DELETE", urlOrHandler, handler),
  head: (urlOrHandler, handler) =>
    Router(routes).register("HEAD", urlOrHandler, handler),
  options: (urlOrHandler, handler) =>
    Router(routes).register("OPTIONS", urlOrHandler, handler),
  ...
  serve: (req, res) => {
    ...
    const matchingUrl = Object.keys(routes[method]).find((key) =>
